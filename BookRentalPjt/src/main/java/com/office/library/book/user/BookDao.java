	...
	public List<RentalBookVo> selectRentalBooks(int u_m_no) {
		...
		try {
			rentalBookVos = jdbcTemplate.query(sql, new RowMapper<RentalBookVo>() {
				@Override
				public RentalBookVo mapRow(ResultSet rs, int rowNum) throws
SQLException {
					...
					rentalBookVo.setB_reg__date(rs.getString("b_reg__date"));
